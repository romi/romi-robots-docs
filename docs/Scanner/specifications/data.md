This page describes how to use the romi package `plantdb` accessible [here](https://github.com/romi/plantdb).
...
!!! warning
   If you intend to contribute to the development of `plantdb` or want to be able to edit the code and test your changes, you should choose _editable mode_.
...
0. OPTIONAL - create a list of RGB images If you do not have a scan datasets available, either download a shared datasets [here](https://db.romi-project.eu/models/test_db.tar.gz) or generate a list of images as follows:
    ```python
    import numpy as np
...
Inside this directory we need to define (add) the `romidb` marker, so it may be used by `FSDB` class.
...
Inside `myscan_001/`, we find the datasets or _fileset_ in `plantdb` terminology.
...
This creates an `images` directory and a `files.json` at the dataset root directory.
We now have the following tree structure:
